...
import { uploadProducts, type UploadResult } from "./actions";

export function ProductsUploader() {
  ...
  async function onSubmit(formData: FormData) {
    ...
    if (!file || file.size === 0) {
      setResult({
        ...
        errors: ["Selecciona un archivo Excel."], newCategories: [], newBrands: [],
      });
      return;
    }
    startTransition(async () => {
      const r = await uploadProducts(formData);
      setResult(r);
    });
  }
        ...
        >
          ...
          {result.newCategories.length > 0 && (
            <p className="mt-3 text-xs">
              <b>Nuevas categorías:</b> {result.newCategories.join(", ")}
            </p>
          )}
          {result.newBrands.length > 0 && (
            <p className="mt-1 text-xs">
              <b>Nuevas marcas:</b> {result.newBrands.join(", ")}
            </p>
          )}
