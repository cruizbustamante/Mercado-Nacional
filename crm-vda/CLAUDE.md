...
- ✅ **Módulo Facturación** (`/facturacion`, rol admin/facturador): lista NV listas para facturar (APROBADO + V°B° OK), KPIs, filtros, detalle expandible, "Ver/imprimir NV". **Scraper de facturacion.cl** (puppeteer-core + @sparticuz/chromium-min en Vercel Functions) que llena la **Vista Previa** y captura el PDF de la prefactura — **modo preview, NUNCA emite el DTE** (hasta validar todo). Visor `/prefactura` con pantalla de carga + PDF embebido. Botón "Marcar facturada" (folio real) → estado FACTURADO. El botón "Prefactura" muestra **✓ verde** cuando ya se generó (`invoice_job_status='PREFACTURA'`; no bloquea regenerar). **Funciona en producción (free/Hobby) con la función en región `gru1` (São Paulo)** para bajar latencia a Chile. Falta: emisión real (botón Emitir Documento), envío por correo.
- ✅ **Ciclo de vida unificado de NV** (toda NV vive en `sales_notes`): `PENDIENTE` (emitida) → `APROBADO` (V°B° dado = pendiente de facturación, aparece en `/facturacion`) → `FACTURADO` (folio) → `DESPACHADO` (módulo Stock/Despacho, pendiente). `RECHAZADO` es rama de aprobación.
- ✅ **Facturar OC de supermercado vía módulo Facturación** (migración 012): la OC ya **no** crea la NV como FACTURADO. En el editor de OC (`oc/[id]`) marcas las **cajas a facturar** y "Generar NV para facturar" → crea NV en **`APROBADO`** (sin V°B°, OC es venta firme), ligada a la OC vía `sales_notes.purchase_order_id` + `sales_note_items.purchase_order_item_id`. Esa NV se factura en `/facturacion`; al marcarla facturada, `marcarFacturada` hace **writeback** → crea `oc_invoices`/`oc_invoice_items` (cajas × `unit_price` OC) y recalcula status OC → alimenta el **cumplimiento**. Guard anti-doble-facturación por cajas ya comprometidas (facturadas o en NV pendiente). Una OC puede generar **varias NV** (parciales). El editor muestra columnas Fact./En NV; el listado de Órdenes muestra badge de estado NV ("NV pendiente"/"Facturada").
...
- **Scraper facturacion.cl** (`src/lib/facturacion/`): puppeteer-core + @sparticuz/chromium-min, corre headless en Vercel Functions. Gotchas: (1) clickear la **Factura de VENTAS** por href `venta/venta.php',33,` — hay otra "Factura" de COMPRAS (`form/compra/`) con igual title que aparece antes en el DOM. (2) RUT: en headless el Tab NO dispara el lookup → además disparar change/blur + clic en lupa `#btnBuscar`. (3) Precio unitario = **NETO BASE por UNIDAD** (`price_net_base` de la NV es por unidad, ej. 1.514 × 48 = 72.672); descuento % si final<base; el ILA lo agrega solo facturacion.cl; flete = SKU **12311231**, cantidad=total unidades. (4) Capturar el PDF de Vista Previa con **CDP `Fetch` domain** (no `Network.getResponseBody` — falla con PDF inline). (5) **Región `gru1`** (vercel.json) para bajar latencia Vercel↔Chile y caber en 60s de Hobby. (6) **Solo modo preview, NUNCA emite** hasta validar. Credenciales en env vars `EMPRESA_FCL`/`USUARIO_FCL`/`PASSWORD_FCL` (server-only). Iterar en **localhost** (headless igual que prod, sin límite 60s).
  - **Referencia del documento** (`build-input.ts`): NV de supermercado (tiene `purchase_order_id`) → **ORDEN DE COMPRA (SII 801)** con N° y fecha de la OC (factura real: "ORDEN DE COMPRA: Nro. 3350728457"). NV manual → NOTA DE PEDIDO (802) con N° de NV.
  - **Tiempos / límite Hobby**: 1 SKU ≈ **40s** (login+nav+referencia+vista previa+captura) + ~**3,3s por SKU**. La entrada (~37s) es casi toda red + cold-start de Chromium (no son sleeps recortables). En frío saca ~3 SKU en 60s, **en caliente ~8** (warm reusa Chromium en `/tmp`). **Hobby 60s NO cabe facturas grandes** (Walmart 17 SKU ≈ 90s). **PENDIENTE: pasar a Vercel Pro y subir `maxDuration` 60→300** en `prefactura` y `emitir` (solo esas 2 rutas corren el scraper). NO intentar "esperas por condición" en la carga de SKU sin confirmar los selectores reales: ya se probó y fue **más lento** (esperaba al tope y bajó de 7 a 4 SKU) → se revirtió a sleeps fijos (900 tras Enter, 700 tras agregar).
